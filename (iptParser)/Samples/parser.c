...
#include "parser.h"		// KAS changed for 8.3 naming purposes
#ifdef	WIN32			// KAS Mac and PC end-of-line sizes different
#define	LINE_ENDCT	5
#else
#define	LINE_ENDCT	4
#endif
...
void main(void)
{
	...
	for (i = 0; keyWords[i].keyWord; ++i) {
		if (i == 0 ||
strcmp(keyWords[i].funcName,keyWords[i-1].funcName)) {
			if ((t++) == 0)
...
short	CountMatches(char *input, short level)
{
	...
	for (i = 0; keyWords[i].keyWord; ++i) {
		if (level == 0 || strncmp(keyWords[i].keyWord,input,level)
== 0) {
			if (!(alphaMatch & (1L <<
keyWords[i].keyWord[level]))) {
				++n;
				alphaMatch |= (1L <<
keyWords[i].keyWord[level]);
			}
	...
	if (CountMatches(keyWords[input].keyWord,level) == 1) {
		if (keyWords[input].keyWord[level] == 0)
			fprintf(ofile, "%*sif (*(sp++) ==\'\\0\')\n%*skeyVal = sf_%s;\n",
				level+Margin, "",level+1+Margin,"",&keyWords[input].funcName[2]);
		else {
			fprintf(ofile, "%*sif (*(sp++) ==\'%c\')\n",
				level+Margin,"",keyWords[input].keyWord[level]);
			GenSwitch(input,(short) (level+1));
		}
	}
	else {
		...
		for (i = 0; keyWords[i].keyWord; ++i) {
			if (level == 0 ||
strncmp(keyWords[i].keyWord,keyWords[input].keyWord,level) == 0) {
				// Match thus far
				if (keyWords[i].keyWord[level] == 0)
					fprintf(ofile, "%*scase \'\\0\':keyVal = sf_%s; break;\n",
						level+Margin,
"",&keyWords[i].funcName[2]);
				else if (!(alphaBits & (1L <<
(keyWords[i].keyWord[level]-'A')))) {
					alphaBits |= (1L <<
(keyWords[i].keyWord[level]-'A'));
					fprintf(ofile, "%*scase \'%c\':\n",
						level+Margin,
"",keyWords[i].keyWord[level]);
						GenSwitch(i,(short) (level+1));
					fprintf(ofile,
"%*sbreak;\n",(level+1)+Margin, "");
				}
			}
...
(keyWords[i].keyWord[level]-'A'));
					...
					if (keyWords[i].keyWord[level] == 0) {
						...
						fprintf(ofile,"%*sP_MatchFork | \'\\0\', 0x??,\n",
							level*2+Margin, "");
						lastOffset = ftell(ofile) - LINE_ENDCT;

						fprintf(ofile,"%*ssf_%s,\n",level*2+Margin, "",
...
void SortKeywords()
{
	...
	for (i = 1; keyWords[i].keyWord; ++i) {
		temp = keyWords[i];
		for (j = i; j > 0 &&
strcmp(temp.keyWord,keyWords[j-1].keyWord) < 0; --j) {
