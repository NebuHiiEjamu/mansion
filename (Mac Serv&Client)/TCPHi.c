...
OSErr CreateStream(unsigned long *stream,unsigned long recvLen)
{
	...
	recvPtr = NewPtrClear(recvLen);	/* 7/1/96 JAB Changed to NewPtrClear */
	err = MemError();
	if (err==noErr)
		err = LowTCPCreateStream(stream,recvPtr,recvLen,(TCPNotifyUPP)nil);
	return err;
}
...
OSErr AsyncOpenConnection(unsigned long stream,long remoteHost,short remotePort,Byte timeout,TCPiopb **returnBlock)
{
	tcp_port localPort = 0;
	
	return LowTCPOpenConnectionAsync(stream,timeout,remoteHost,remotePort,
								localPort,returnBlock);
}

OSErr FinishOpenConnection(TCPiopb *returnBlock)		// 6/15/95 JAB
{
	ip_addr localHost;
	tcp_port localPort;
	
	return LowFinishTCPOpenConn(returnBlock,&localHost,&localPort);
}
...
/* JAB - renamed to MyCloseConnection to avoid conflict with UniversalHeaders CodeFragments.h */
...
OSErr SendMultiData(unsigned long stream,Str255 data[],short numData,Boolean retry)
{
	...
	theWDS = (wdsEntry *)NewPtrClear((numData+1) * sizeof(wdsEntry));
	if (MemError())
		return MemError();	
...
{	
	...
	InitNetCursor();
	theWDS = (wdsEntry *)NewPtrClear( (2*sizeof(wdsEntry)) );
