...
#
# Jim - changed ulpTimeOut Actions to 0, which according to Apple's
# MACTCP Q&A means report timeouts, rather than abort connection.
#
#
#-----------------------------------------------------------*/
...
OSErr LowTCPWaitForConnection(StreamPtr streamPtr,b_8 timeout,ip_addr *remoteHost,
			...
			Boolean async,TCPiopb **returnBlock)
{
	...
	pBlock->csParam.open.ulpTimeoutAction = 1;	/* JAB 9/7/95 0 means report, rather than sever */
...
OSErr LowTCPOpenConnection(StreamPtr streamPtr,b_8 timeout,ip_addr remoteHost,
			tcp_port remotePort,ip_addr *localHost,tcp_port *localPort)
{
	OSErr err;
	TCPiopb *pBlock;
...
OSErr LowTCPOpenConnectionAsync(StreamPtr streamPtr,b_8 timeout,ip_addr remoteHost,
...
OSErr LowTCPSendData(StreamPtr streamPtr,b_8 timeout,Boolean push,Boolean urgent,
...
{	
	...
	pBlock->csParam.send.ulpTimeoutAction = 0;	/* JAB 9/7/95 0 means report, rather than sever */
...
OSErr LowTCPNoCopyRcv(StreamPtr streamPtr,b_8 timeout,Boolean *urgent,Boolean *mark,
...
OSErr LowTCPRecvData(StreamPtr streamPtr,b_8 timeout,Boolean *urgent,Boolean *mark,
...
OSErr LowTCPClose(StreamPtr streamPtr,b_8 timeout)
{
	OSErr err;
	TCPiopb *pBlock;
	...
	pBlock->csParam.close.ulpTimeoutAction = 1;	/* JAB 9/7/95 0 means report, rather than sever */
	PBControl((ParmBlkPtr)pBlock,true);
	while (pBlock->ioResult > 0 && GiveTime(cSleepTime))
		;
	if (gCancel)
...
OSErr GetMyIP(ip_addr *ipnum)
{
	...
	/* 7/1/96 JAB Changed to NewPtrClear */
	IPBlock = (struct GetAddrParamBlock *)NewPtrClear(sizeof(struct GetAddrParamBlock));
	if (MemError() != noErr)
		return MemError();
