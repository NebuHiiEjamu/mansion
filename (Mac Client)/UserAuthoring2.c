...
void RoomInfoDialog()
{
	...
	SetControl(dp, R_Private, (gRoomInfo.flags & RF_Private) > 0);
	SetControl(dp, R_NoPainting, (gRoomInfo.flags & RF_NoPainting) > 0);
	SetControl(dp, R_NoCyborgs, (gRoomInfo.flags & RF_CyborgFreeZone) > 0);
...
//		ModalDialog(filterProc, &itemHit);
	...
	case R_LockItem:
		...
		break;
	case R_Private:
		gRoomInfo.flags ^= RF_Private;
		SetControl(dp, R_Private, (gRoomInfo.flags & RF_Private) > 0);
		break;
	case R_NoPainting:
		gRoomInfo.flags ^= RF_NoPainting;
		SetControl(dp, R_NoPainting, (gRoomInfo.flags & RF_NoPainting) > 0);
		break;
	case R_NoCyborgs:
		gRoomInfo.flags ^= RF_CyborgFreeZone;
		SetControl(dp, R_NoCyborgs, (gRoomInfo.flags & RF_CyborgFreeZone) > 0);
		break;
	}
