...
void MemberRoomModification(ServerUserPtr cUser, int modNumber, char *arg)
{
	...
	case MR_Title:
		if (*arg && LegalRoomName(arg)) {
			char oldName[256],newName[256];			/* 1/3/97 JAB */
			BeginRoomCreation();
			CopyRoom(&cRoom->room);
			strcpy(oldName,CvtToCString((StringPtr) &gRoom->varBuf[gRoom->roomNameOfst]));			/* 1/3/97 JAB */
			/* affect the change */
			gRoom->roomNameOfst = AddRoomCtoPString(arg);
			strcpy(newName,CvtToCString((StringPtr) &gRoom->varBuf[gRoom->roomNameOfst]));			/* 1/3/97 JAB */
			newRoom = EditRoomToNewRoom();
			EndRoomCreation();
			ReplaceRoom(cRoom,newRoom,true);
			UserMessage(cUser,"Room name changed to %s",arg);
			/* 1/3/97 JAB */
			TimeLogMessage("%s changed name of room \"%s\" to \"%s\"\r",
						CvtToCString(cUser->user.name),oldName,newName);
		}
		else
